        ...
        if unique_name:
            ...
            if unique_abbreviation:
                ...
                if unique_chair_name:
                    ...
                    if unique_building:
                        office_count: int = collection.count_documents({"office": office})
                        unique_office = office_count == 0
                        if not unique_office:
                            print("We already have a department with that office number.  Try again.")
                            if unique_office:
...
if __name__ == '__main__':
    ...
    if 'departments_building' in departments_indexes.keys():
        print("building index present.")
    else:
        departments.create_index([('building', pymongo.ASCENDING)], unique=True,
                                 name='departments_building')
    pprint(departments.index_information())

    if 'departments_office' in students_indexes.keys():
        print("office index present.")
    else:
        departments.create_index([('office', pymongo.ASCENDING)], unique=True,
